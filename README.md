...
```bash
make install
# 等价于: cd web && pnpm install && cd ../server && uv sync
```
...
# E2E 集成测试（需先启动服务）
cd server && python3 tests/e2e/test_integration.py

# 数据库迁移
cd server && uv run alembic revision --autogenerate -m "describe changes"
...
| Deploy | Docker Multi-stage / uv / pnpm / Alembic |
