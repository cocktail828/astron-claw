...
# 配置环境
cd server
cp .env.example .env    # 编辑 .env，填写 MySQL 和 Redis 连接信息
...
| 地址 | 说明 |
|------|------|
| `http://localhost:8765/` | Chat 聊天界面 |
| `http://localhost:8765/admin` | Admin 管理面板（首次需设置密码） |
...
docker run -d --name astron-claw -p 8765:8765 \
...
| `SERVER_WORKERS` | Worker 进程数 | `CPU cores + 1` |
...
```
server/
├── infra/          # 基础设施：Config / MySQL / Redis / ORM / Storage / Log
├── services/       # 业务逻辑：Bridge / Queue / Session / Token / Media / Auth
├── routers/        # 路由层：SSE / WebSocket / REST API
...
# E2E 集成测试（需先启动服务）
python3 tests/e2e/test_integration.py
...
| 层级 | 技术 |
|------|------|
| Server | Python 3.11+ / FastAPI / Uvicorn (uvloop) |
| Storage | MySQL (SQLAlchemy Async ORM) / Redis Streams / S3 |
| Plugin | TypeScript / WebSocket / OpenClaw SDK |
| Frontend | Vanilla HTML + CSS + JS |
| Protocol | HTTP SSE + WebSocket + JSON-RPC 2.0 |
| Deploy | Docker Multi-stage / uv / Alembic |
